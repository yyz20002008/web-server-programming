...
function GetProduct($id)
{
        ...
        $conn = getConnection();
        $result = $conn->query("SELECT * FROM Product P Where P.pro_id=$id");
...
{
        $conn = getConnection();
        $result = $conn->query('SELECT * FROM Product P');
        return $result;
}

function Placeorder($tranNum,$amount,$address)
{
	$conn = getConnection();
    $reset = $conn->query("ALTER TABLE OrderView AUTO_INCREMENT = 1");
    $sql="INSERT INTO  `N02202273_db`.`OrderView` (trans_number,amount,address)  VALUES ('$tranNum',$amount,'$address') ";
   // echo $sql;    
...
function GetOrder()
{
       $conn = getConnection();
        ...
        $result = $conn->query('SELECT * FROM OrderView O');
        return $result;
